...
def train_model(args, model_sturcture, train_set, test_set ,parameters):
    ...
    for run_id, (lr, embed_dim, batch_size, layer_num,hidden_dim,readout, use_fp, optimizer_name) in enumerate(product(*param_values)):    
        ...
        comment = f' batch_size = {batch_size} lr = {lr} embed_dim = {embed_dim} layer_num = {layer_num} readout = {readout} hidden_dim = {hidden_dim} use_fp = {use_fp} optimizer_name = {optimizer_name}'
        ...
        tb.add_hparams(
            {"lr": lr, "bsize": batch_size, "embed_dim": embed_dim, "layer_num": layer_num,'hidden_dim' : hidden_dim,'readout': readout, 'best_epoch' : best_epoch, 'optimizer_name': optimizer_name },
            {
