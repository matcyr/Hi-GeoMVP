...
class single_omics_atten(nn.Module):
    def __init__(self, model_config) -> None:
        ...
        self.fc1 = nn.Linear(self.graph_dim, 1024)
...
def train_model(args, train_set, test_set ,parameters):
    ...
    device = args.device
    for run_id, (lr, embed_dim, batch_size, layer_num,hidden_dim,readout, use_fp, attention_dim, optimizer_name, cell_type) in enumerate(product(*param_values)):    
        ...
        model_config = {'embed_dim': embed_dim, 'dropout_rate': args.dropout_rate,'hidden_dim' : hidden_dim,
                        'layer_num': layer_num, 'readout': readout, 'use_cnn' : True, 'use_fp' : use_fp, 'use_smiles' : False, 'attention_dim' : attention_dim, 'cell_type' : cell_type}
