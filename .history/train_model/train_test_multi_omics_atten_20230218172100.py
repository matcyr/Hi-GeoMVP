...
def train_model(args, model_sturcture, train_set, test_set ,parameters):
    ...
    device = args.device
    for run_id, (lr, embed_dim, batch_size, layer_num,hidden_dim,readout, use_fp, optimizer_name, atten_dim) in enumerate(product(*param_values)):    
        ...
        model_config = {'embed_dim': embed_dim, 'dropout_rate': args.dropout_rate,'hidden_dim' : hidden_dim,
                        'layer_num': layer_num, 'readout': readout, 'use_cnn' : True, 'use_fp' : use_fp, 'use_smiles' : False, 'atten_dim' : atten_dim}
        ...
        comment = f' batch_size = {batch_size} lr = {lr} embed_dim = {embed_dim} layer_num = {layer_num} readout = {readout} hidden_dim = {hidden_dim} use_fp = {use_fp} optimizer_name = {optimizer_name} atten_dim = {atten_dim}'
        print('Begin Training')
        print(f'Embed_dim_drug : {embed_dim}'+ '\n' +f'Hidden_dim_cell : {hidden_dim} \n' +  f'layer_num : {layer_num} \n'+ 
              f'read_out_function : {readout}\n' + f'use_fp : {use_fp}\n' +f'batch_size : {batch_size}\n' + f'optimizer : {optimizer_name}\n' + f'lr : {lr}\n' + f'atten_dim : {atten_dim}\n' )
        ...
        tb.add_hparams(
            {"lr": lr, "bsize": batch_size, "embed_dim": embed_dim, "layer_num": layer_num,'hidden_dim' : hidden_dim,'readout': readout, 'best_epoch' : best_epoch, 'optimizer_name': optimizer_name, 'use_fp' : use_fp},
            {
