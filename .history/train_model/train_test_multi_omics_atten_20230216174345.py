...
def train_multi_omics_atten_step(model, train_loader, optimizer, loss_op, writer, epoch,device):
    model.train()
    y_true, preds = [], []
    optimizer.zero_grad()
...
def train_model(args, model_sturcture, train_set, test_set ,parameters, optimizer_name):
    save_dir = 'Atten_model_multi_omics_'+ args.train_type
    ...
    device = args.device
    for run_id, (lr, embed_dim, batch_size, layer_num,hidden_dim,readout, use_fp) in enumerate(product(*param_values)):    
        ...
        elif:optimizer = opt.SGD(model.parameters(), lr=lr, momentum=0.9, weight_decay=1e-2)
        ...
        comment = f' batch_size = {batch_size} lr = {lr} embed_dim = {embed_dim} layer_num = {layer_num} readout = {readout} hidden_dim = {hidden_dim} use_fp = {use_fp}'
        ...
        tb.add_hparams(
            {"lr": lr, "bsize": batch_size, "embed_dim": embed_dim, "layer_num": layer_num,'hidden_dim' : hidden_dim,'readout': readout, 'best_epoch' : best_epoch },
            {
