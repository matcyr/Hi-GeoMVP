...
def build_graph(nodes, avg_edges_per_node=5):
    similarities = cosine_similarity(nodes)
    # Threshold similarities to create adjacency matrix
    threshold = np.percentile(similarities, 100 - 100 / avg_edges_per_node)
...
def create_cell_data():
    ...
    if not torch_geometric.utils.contains_self_loops(exp_edge_list):
        exp_edge_list = torch_geometric.utils.add_self_loops(exp_edge_list)[0]
        print(f'Gene graph has self loop: {torch_geometric.utils.contains_self_loops(exp_edge_list)}')
    ...
    for cell_id in copy_number.index:
        ...
        for gene in copy_number.columns:
            ...
            x_feat.append(feat)
        x_feat = np.asarray(x_feat)
        x_feat_CNV[cell_id] = torch.tensor(x_feat, dtype=torch.float)
    ...
    for cell_id in mutation.index:
        ...
        for gene in mutation.columns:
            ...
            x_feat.append(feat)
        x_feat = np.asarray(x_feat)
        x_feat_mut[cell_id] = torch.tensor(x_feat, dtype=torch.float)
