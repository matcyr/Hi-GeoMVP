...
def total_loss(class_drug,repr_drug, target, threshold, lamda = 1.0):
    return 
device = torch.device('cuda:0' if torch.cuda.is_available() else 'cpu')lamda * criterion_class(class_drug,target) + criterion_regre(repr_drug,threshold)
...
def train(model, train_loader, optimizer, loss_op, device):
    ...
    for data in train_loader:
        with torch.autocast(device_type='cuda', dtype=torch.float16, enabled=use_amp):
            ...
            loss = loss_op(drug_class, drug_regr, target, threshold).float()
