...
class SelfAttention(nn.Module):
    def __init__(self, embed_dim, hidden_dim):
        ...
        self.values = nn.Linear(1, self.embed_dim)
        self.keys = nn.Linear(1, self.embed_dim)
        self.queries = nn.Linear(1, self.embed_dim)
...
class TransformerBlock(nn.Module):
    ...
    def forward(self, value, key, query):
        values = value.unsqueeze(dim=-1)
        keys = key.unsqueeze(dim=-1)
        queries = query.unsqueeze(dim = -1)
        attention = self.attention(value, key, queries)
