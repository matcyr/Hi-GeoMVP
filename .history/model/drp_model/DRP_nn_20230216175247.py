...
class Multi_Omics_TransRegression(nn.Module):
    def __init__(self, model_config):
        ...
        self.gene_attn = TransformerBlock(self.graph_dim, self.hidden_dim)
        self.mut_attn = TransformerBlock(self.graph_dim, self.hidden_dim)
        self.cnv_attn = TransformerBlock(self.graph_dim, self.hidden_dim)
        self.regression = nn.Sequential(
            nn.Linear(3*self.graph_dim, 1024),
            nn.ELU(),
            nn.Dropout(p=self.dropout_rate),
            nn.Linear(1024, self.graph_dim,),
            nn.ELU(),
            nn.Dropout(p=self.dropout_rate),
