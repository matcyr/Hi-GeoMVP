...
class SelfAttention(nn.Module):
    def __init__(self, model_config):
        ...
        self.values = nn.Linear(self.hidden_dim, self.embed_dim)
        self.keys = nn.Linear(self.embed_dim, self.embed_dim)
        self.queries = nn.Linear(self.hidden_dim, self.embed_dim)
        self.fc_out = nn.Linear(self.embed_dim, self.embed_dim)
    def forward(self, values, keys, queries ): ## Cell, Drug, Cell
        N = queries.shape[0]
        values = self.values(values)
        keys = self.keys(keys)
        queries = self.queries(queries)
        ## Split embedding into self.num_head pieces
        values = values.reshape(N, self.num_head, self.head_dim)
        keys = keys.reshape(N , self.num_head, self.head_dim)
        queries = queries.reshape(N, self.num_head, self.head_dim)
        energy = torch.einsum("nhd,nhd->nh", [queries, keys])
        attention = torch.softmax(energy / (self.embed_dim ** (1/2)), dim = 1)
        out = torch.einsum("nh,nhd->nhd", [attention, values]).reshape(N, self.embed_dim)
        out = self.fc_out(out) 
        return out
...
class TransformerBlock(nn.Module):
    def __init__(self, model_config):
        ...
        self.attention = SelfAttention(model_config)
        self.embed_size = model_config.get('embed_dim')
        ...
        self.dropout = nn.Dropout(model_config.get('dropout_rate'))
...
class Multi_Omics_TransRegression(nn.Module):
    def __init__(self, model_config):
        ...
        self.use_smiles = model_config.get('use_smiles')
        self.use_fp = model_config.get('use_fp')
        self.gene_attn = TransformerBlock(model_config)
        self.mut_attn = TransformerBlock(model_config)
        self.cnv_attn = TransformerBlock(model_config)
        self.regression = nn.Sequential(
    ...
    def forward(self, drug_atom, drug_bond, ge, cell_cnv, cell_mut):
        ...
        mut_embed = self.cell_encoder_mut(cell_mut.x,cell_mut.edge_index,cell_mut.batch)
        cnv_embed = self.cell_encoder_cnv(cell_cnv.x,cell_cnv.edge_index,cell_cnv.batch) 
        attended_gene_feat = self.gene_attn(ge_embed, drug_embed, ge_embed)
        attended_mut_feat = self.mut_attn(mut_embed, drug_embed, mut_embed)
        attended_cnv_feat = self.cnv_attn(cnv_embed, drug_embed, cnv_embed)
        attended_feat = torch.cat([attended_gene_feat, attended_mut_feat, attended_cnv_feat], dim=-1)
        x = self.regression(attended_feat)
        return x