...
def _collate_drug_class(samples):
    ...
    batched_target = torch.cat(target, dim = 1)
    return batched_drug_atom, batched_drug_bond, batched_target, torch.tensor(threshold) ## batched_target : []
...
if __name__ == '__main__':
    ...
    train_loader = drug_class_loader(train_drug_set, batch_size = 32)
    da,db,tar,ic = next(iter(train_loader))
