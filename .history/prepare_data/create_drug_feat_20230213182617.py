...
### drug 174:RDKit ERROR: [22:42:02] UFFTYPER: Unrecognized charge state for atom: 54
    ...
    drug_name = list(drug_name_id_dict.keys())
    target_drug = {drug:target_drug.loc[drug].values for drug in drug_name}
    ...
    def save_drug_feat():
        ...
        np.save(os.path.join(save_path, 'target_drug.npy'), target_drug)
...
def load_drug_feat():
    ...
    target_drug = np.load('./Data/DRP_dataset/target_drug.npy',allow_pickle=True).item()
    pathway = np.load('./Data/DRP_dataset/pathway.npy')
    return drug_atom_feat, drug_bond_feat, target_drug, pathway
if __name__ =='__main__':
    root = os.getcwd()
    if os.path.exists(root + f"/Data/DRP_dataset") is False:
        os.makedirs(root + "./Data/DRP_dataset")
    drug_atom_dict,drug_bond_dict, target_drug, pathway = create_drug_dict()
