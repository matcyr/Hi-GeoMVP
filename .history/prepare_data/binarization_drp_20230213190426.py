...
def binarization_drug_resposne():        
    ...
    t = 0.05
    norm_t = norm.ppf(t)        
    def Get_threshold_drug(drug_id, drug_ic50_dict):
        tmp = np.array(drug_ic50_dict[drug_id])
        kde.fit(tmp[:, None])
        min_ic50 = np.min(tmp)
        max_ic50 = np.max(tmp)
    ...
    drug_response_dict = drug_response_dict, drug_name, cell_name = read_dr_dict()
    drug_ic50_dict = drug_ic50_upsampling(drug_response_dict)
    drug_threshold = {drug_id : Get_threshold_drug(drug_id,drug_ic50_dict) for drug_id in drug_name_id_dict.keys()}
