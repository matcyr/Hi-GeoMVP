...
train_idx,create_drp_set(type= args.train_type, drug_name = drug_name, cell_name = cell_name, drug_response_dict = drug_response_dict, seed = 0)
class multi_DRP_dataset(Dataset):
    def __init__(self, use_norm_ic50 = 'True'):
        super(multi_DRP_dataset, self).__init__()
        self.drug_atom, self.drug_bond_dict, self.ge_dict, self.cnv_dict, self.mut_dict, self.DR = drug_atom_dict, drug_bond_dict, ge_HN_feat, cnv_dict, mut_dict, drug_response_dict
...
class single_DRP_dataset(Dataset):
    def __init__(self, omics = 'ge', use_norm_ic50 = 'True'):
        super(single_DRP_dataset, self).__init__()
        if omics == 'ge':
            self.drug_atom, self.drug_bond_dict, self.ge_dict, self.DR = drug_atom_dict, drug_bond_dict, ge_HN_feat, drug_response_dict
        elif omics == 'cnv':
            self.drug_atom, self.drug_bond_dict, self.ge_dict, self.DR = drug_atom_dict, drug_bond_dict, cnv_dict, drug_response_dict
        elif omics == 'mut':
            self.drug_atom, self.drug_bond_dict, self.ge_dict, self.DR = drug_atom_dict, drug_bond_dict, mut_dict, drug_response_dict
        self.use_norm_ic50 = use_norm_ic50
        self.length = len(self.DR)
    def __len__(self):
        return self.length
