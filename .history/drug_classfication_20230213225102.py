...
def total_loss(class_drug,repr_drug, target, threshold, lamda = 1.0):
    return lamda * criterion_class(class_drug,target) + criterion_regre(repr_drug,threshold)
# criterion_regre = mse_loss
# criterion_class(graph_pred_class, tar)
# criterion_regre(graph_pred_regr, ic)
...
train_loader = drug_class_loader(train_drug_set, batch_size = 1)
test_loader = drug_class_loader(test_drug_set, batch_size = 200)

# %%
drug_atom, drug_bond, target,threshold = next(iter(train_loader))

# %%
drug_class, drug_regr = model(drug_atom)
loss = criterion(drug_class, drug_regr, target, threshold)
