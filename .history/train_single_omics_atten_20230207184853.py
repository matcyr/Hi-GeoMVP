...
from model.drp_model.DRP_nn import Single_Omics_Regression
...
def arg_parse():
    ...
    parser.add_argument('--use_norm_ic50',type = bool, default= True)
...
def train(model, train_loader, optimizer, loss_op, writer, epoch,device):
    ...
    for data in tqdm(train_loader):
        drug_atom, ge, ge_sim,ic50 = data
        drug_atom = drug_atom.to(device)  
        ge = ge.to(device)
        ge_sim = ge_sim.to(device)
        ic50 = ic50.to(device)  
        # with torch.cuda.amp.autocast():
        loss = loss_op(model(drug_atom, ge, ge_sim), ic50.view(-1, 1).float())
...
def test(model,loader,device):
    ...
    for data in tqdm(loader):
        drug_atom, ge, ge_sim,ic50 = data
        drug_atom, ge, ge_sim,ic50 = drug_atom.to(device), ge.to(device), ge_sim.to(device), ic50.to(device)
        y_true.append(ic50.view(-1, 1).float())
        out = model(drug_atom, ge, ge_sim)
...
class single_DRP_dataset(Dataset):
    def __init__(self, drug_atom_dict, ge_dict, ge_sim_dict, drug_response_dict, use_norm_ic50 = True):
        super(single_DRP_dataset, self).__init__()
        self.drug_atom, self.ge_dict, self.ge_sim_dict, self.DR = drug_atom_dict, ge_dict, ge_sim_dict, drug_response_dict
        self.use_norm_ic50 = use_norm_ic50
        self.length = len(self.DR)
    def __len__(self):
        return self.length
    def __getitem__(self, index):
        if self.use_norm_ic50:
            cell, drug, _, ic = drug_response_dict[index]
        else: cell, drug, ic, _ = drug_response_dict[index]
        return (self.drug_atom[drug], self.ge_dict[cell], self.ge_sim_dict[cell], ic)
def _collate(samples):
    drug_atom, ge, ge_sim, labels = map(list, zip(*samples))
    batched_drug_atom = Batch.from_data_list(drug_atom)
    batch_ge = Batch.from_data_list(ge)
    batch_ge_sim = Batch.from_data_list(ge_sim)
    return batched_drug_atom, batch_ge, batch_ge_sim, torch.tensor(labels)
...
if __name__ == '__main__':
    ...
    train_set = single_DRP_dataset(drug_atom_dict=drug_atom_dict, ge_dict=ge_HN_feat, ge_sim_dict= ge_sim_dict, drug_response_dict=drug_response_dict[train_idx],use_norm_ic50= args.use_norm_ic50)
    test_set = single_DRP_dataset(drug_atom_dict=drug_atom_dict, ge_dict=ge_HN_feat, ge_sim_dict= ge_sim_dict, drug_response_dict=drug_response_dict[test_idx],use_norm_ic50= args.use_norm_ic50)
    device  = torch.device("cuda:"+str(args.device) if torch.cuda.is_available() else "cpu") 
    if args.use_norm_ic50:
        weights = 'weights'
    else: weights = 'weights_norm'
    ...
    def train_model(parameters,model_sturcture):
        ...
        for run_id, (lr, embed_dim, batch_size, layer_num,hidden_dim,readout,atten_dim) in enumerate(product(*param_values)):      
            ...
            model_config = {'embed_dim': embed_dim, 'dropout_rate': 0.4,'hidden_dim' : hidden_dim,
                            'layer_num': layer_num, 'readout': readout, 'use_cnn' : True, 'atten_dim' : atten_dim, 'use_fp' : False, 'use_smiles' : False}
            model = model_sturcture(model_config).to(device)
            optimizer = opt.Adam(model.parameters(), lr=lr, weight_decay=0.005)
        ...
        tb.close()
    train_model(parameters,Single_Omics_Regression)