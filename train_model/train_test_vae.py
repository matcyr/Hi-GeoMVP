...
def _collate_single_DRP(samples):
    ...
    return batched_drug_atom, batched_drug_bond, torch.stack(ge).float(), torch.tensor(labels)
def vae_drp_loader(data_set,batch_size,shuffle = True, num_workers = 4):
...
def train_single_omics_atten_step(model, train_loader, optimizer, loss_op, writer, epoch,device, regular = 'False'):
    model.train()
    y_true, preds = [], []
    optimizer.zero_grad()
    for data in tqdm(train_loader):
        drug_atom,drug_bond,cell_ge, ic50 = data
        ...
        pred, loss_vae = model(drug_atom,drug_bond,cell_ge)
        preds.append(pred.float().cpu())
        if regular == 'True':
            loss = loss_op(pred, ic50.view(-1, 1).float()) + 1e-6 * loss_vae
        else: loss = loss_op(pred, ic50.view(-1, 1).float())
...
def test_single_omics_atten_step(model,loader,device):
    ...
    for data in tqdm(loader):
        drug_atom,drug_bond,cell_ge, ic50 = data
        drug_atom,drug_bond,cell_ge, ic50 = data = drug_atom.to(device), drug_bond.to(device), cell_ge.to(device), ic50.to(device)
        y_true.append(ic50.view(-1, 1).float())
        out, loss_vae = model(drug_atom,drug_bond,cell_ge)
...
def train_model(args, train_set, test_set ,parameters):
    save_dir = 'tensorboard_vae_'+ args.train_type
    ...
    device = args.device
    for run_id, (lr, embed_dim, batch_size, layer_num,hidden_dim,readout, use_fp, optimizer_name, regular) in enumerate(product(*param_values)):    
        if regular == 'True':
            save_dir = save_dir + regular
        ...
        model_config = {'embed_dim': embed_dim, 'dropout_rate': args.dropout_rate,'hidden_dim' : hidden_dim,
                        'layer_num': layer_num, 'readout': readout, 'use_cnn' : True, 'use_fp' : use_fp, 'use_smiles' : False}
        model = Drug_3d_VAE_resp(model_config).to(device)
        ...
        comment = f' batch_size = {batch_size} lr = {lr} embed_dim = {embed_dim} layer_num = {layer_num} readout = {readout} hidden_dim = {hidden_dim}  optimizer_name = {optimizer_name} '
        ...
        train_loader = vae_drp_loader(train_set,batch_size= batch_size,shuffle = True, num_workers = args.num_workers)
        test_loader = vae_drp_loader(test_set,batch_size= 512,shuffle = True, num_workers = args.num_workers)
        n_epochs = args.epochs
        epoch_len = len(str(n_epochs))
        best_val_pcc = -1
        if args.use_norm_ic50 == 'True':
            results = 'results_vae_norm'
        else: results = 'results_vae'
        if os.path.exists(f"./{results}/{args.train_type}") is False:
            os.makedirs(f"./{results}/{args.train_type}")
        for epoch in range(n_epochs):
            train_rmse, train_pcc = train_single_omics_atten_step(model, train_loader, optimizer, criterion, tb, epoch, device, regular)
            # scheduler.step()
        ...
        tb.add_hparams(
            {"lr": lr, "bsize": batch_size, "embed_dim": embed_dim, "layer_num": layer_num,'hidden_dim' : hidden_dim,'readout': readout, 
             'best_epoch' : best_epoch, 'optimizer_name': optimizer_name, 'use_fp' : use_fp},
            {
