...
class MUT_model(nn.Module):
    ...
    def forward(self, cell_mut):
        ...
        if self.cell_conv_type == 'cnn':
            ...
            xt = self.cell_encoder(cell_mut.x, cell_mut.edge_index, cell_mut.batch)
        return xt
        
class CNV_model(nn.Module):
    def __init__(self, model_config) -> None:
        super(CNV_model, self).__init__()
        self.hidden_dim = model_config.get('hidden_dim')
        self.dropout_rate = model_config.get('dropout_rate')
        self.cell_conv_type = model_config.get('cell_conv_type')  ## cnn, gnn
        if self.cell_conv_type == 'cnn':
            self.cell_encoder = cell_cnn(model_config, 2816)
        elif self.cell_conv_type == 'gnn':
            self.cell_encoder = Cell_encoder_cnv(model_config)
        # elif self.cell_conv_type == 'both':
        #     self.cell__cnn_encoder = cell_cnn(model_config, 2560)
        #     self.cell__gnn_encoder = Cell_encoder_mut(model_config)
    def forward(self, cell_cnv):
        batch_size = int(cell_cnv.x.shape[0]/694)
        if self.cell_conv_type == 'cnn':
            xt = self.cell_encoder(cell_cnv.x.view(batch_size,1,694))
        else:
            xt = self.cell_encoder(cell_cnv.x,cell_cnv.edge_index,cell_cnv.batch)
        return xt

class single_omics_atten(nn.Module):
    def __init__(self, model_config) -> None:
        super().__init__()
        self.cell_type = model_config.get('cell_type')
        self.hidden_dim = model_config.get('hidden_dim')
        ...
        if self.cell_type == 'mut':
            self.cell_encoder = MUT_model(model_config)
        elif self.cell_type == 'cnv':
            self.cell_encoder = CNV_model(model_config)
        self.dropout = nn.Dropout(self.dropout_rate)
        self.relu = nn.ReLU()
        self.graph_dim = self.drug_encoder.graph_dim
        self.cell_attn = TransformerBlock(self.graph_dim, self.hidden_dim, self.attention_dim)
        self.fc1 = nn.Linear(self.graph_dim * self.attention_dim, 1024)
        self.fc2 = nn.Linear(1024, 128)
        self.out = nn.Linear(128, 1)
    def forward(self, drug_atom,drug_bond,cell_ge,cell_cnv,cell_mut):
        if self.cell_type == 'mut':
            cell_embed = self.cell_encoder(cell_mut)
        elif self.cell_type == 'cnv':
            drug_embed = self.cell_encoder(cell_cnv)
        drug_embed = self.drug_encoder(drug_atom,drug_bond)
        atten_feat = self.cell_attn(cell_embed, cell_embed, drug_embed)
        out = self.relu(atten_feat)
...
@torch.no_grad()
def test_single_omics_atten_step(model,loader,device):
    model.eval()
    y_true, preds = [], []
...
def train_model(args, train_set, test_set ,parameters):
    save_dir = 'Atten_single_omics_'+ args.train_type
    ...
    device = args.device
    for run_id, (lr, embed_dim, batch_size, layer_num,hidden_dim,readout, use_fp, attention_dim, optimizer_name, cell_type,cell_conv_type) in enumerate(product(*param_values)):    
        save_dir = save_dir + '_' + cell_type
        ...
        model_config = {'embed_dim': embed_dim, 'dropout_rate': args.dropout_rate,'hidden_dim' : hidden_dim,
                        ...
                        'cell_type' : cell_type, 'cell_conv_type' : cell_conv_type}
        model = single_omics_atten(model_config).to(device)
        if optimizer_name == 'Adam':
            optimizer = opt.Adam(model.parameters(), lr=lr, weight_decay=1e-2)
        ...
        comment = f' batch_size = {batch_size} lr = {lr} embed_dim = {embed_dim} layer_num = {layer_num} readout = {readout} hidden_dim = {hidden_dim} cell_type = {cell_type} optimizer_name = {optimizer_name} attention_dim = {attention_dim}'
        ...
        print(f'Embed_dim_drug : {embed_dim}'+ '\n' +f'Hidden_dim_cell : {hidden_dim} \n' +  f'layer_num : {layer_num} \n'+ 
              ...
              f'cell_type : {cell_type}\n' + f'cell_conv_type : {cell_conv_type}')
        ...
        best_val_pcc = -1
        if args.use_norm_ic50 == 'True':
            results = 'results_norm_' + cell_type
        else: results = 'results_' + cell_type
        if os.path.exists(f"./{results}/{args.train_type}") is False:
            os.makedirs(f"./{results}/{args.train_type}")
        for epoch in range(n_epochs):
            train_rmse, train_pcc = train_single_omics_atten_step(model, train_loader, optimizer, criterion, tb, epoch, device)
            # scheduler.step()
            ...
            if epoch % 10 == 0:
                val_rmse,val_pcc, val_r_2, val_mae = test_single_omics_atten_step(model,test_loader, device)
                ...
                with open(reult_file_path,'w') as f:
                    f.write('\n'.join(map(str,[f'embed_dim: {embed_dim}',f'hidden_dim: {hidden_dim}',f'layer_num:{layer_num}', f'read_out: {readout}',
                                            ...
                                            f'cell_conv_type : {cell_conv_type}', f'cell_type : {cell_type}']))) 
