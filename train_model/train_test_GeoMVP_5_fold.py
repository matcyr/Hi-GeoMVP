...
def train_step(model, train_loader, optimizer, writer, epoch, device, args):
    ...
    for data in tqdm(train_loader):
        ...
        with amp.autocast():
            ...
            else: out_dict = model(drug_atom,drug_bond, cell_ge, cell_mut, cell_cnv)
            out = out_dict['pred']
...
@torch.no_grad()
def test_step(model,loader,device, args):
    model.eval()
    y_true, preds = [], []
    for data in tqdm(loader):
        if args.use_raw_gene == 'True':
            ...
            drug_atom, drug_bond, cell_ge, cell_cnv, cell_mut, ic50 = data
            drug_atom, drug_bond, cell_ge, cell_cnv, cell_mut, ic50  = data = drug_atom.to(device), drug_bond.to(device), cell_ge.to(device), cell_cnv.to(device),cell_mut.to(device), ic50.to(device)
            out_dict = model(drug_atom,drug_bond, cell_ge, cell_mut, cell_cnv)
        y_true.append(ic50.view(-1, 1).float())
        out = out_dict['pred']
        preds.append(out.float().cpu())
    ...
    for epoch in range(n_epochs):
        if early_stop_count < args.early_stop_count :
            ...
            if epoch % args.check_step == 0:
                val_rmse,val_pcc, val_r_2, val_mae = test_step(model, val_loader, device, args)
                if args.scheduler_type == 'OP':
                    scheduler.step(val_rmse)
                tb.add_scalar('Accuracy/val/pcc', val_pcc, epoch )
                tb.add_scalar("Accuracy/val/rmse", val_rmse, epoch)
                tb.add_scalar("Accuracy/val/mae", val_mae, epoch)
                tb.add_scalar("Accuracy/val/r_2", val_r_2, epoch)
                ...
                if val_rmse < best_val_rmse or val_pcc > best_val_pcc:
                    ...
                    best_mae = val_mae
                    best_epoch = epoch
                    test_rmse, test_pcc, test_r_2, test_mae = test_step(model,test_loader, device, args)
                    torch.save({
                            'epoch': epoch,
